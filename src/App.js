...
function App() {
  ...
  const addJob = (fstName,lstName,emailId,contactNo,address,District,state,Wagers,work) => {
    let sno;
    if (jobs.length === 0) {
      sno = 0;
    }
  ...
  const onDelete2 = (Agri) => {
    setAgris(agris.filter((e) => {
      return e !== Agri;
    }));
    localStorage.setItem("agris", JSON.stringify(agris));
  }

  const addAgri = (fstName,lstName,emailId,contactNo,address,District,state,NumberofMachine,machine) => {
    let sno;
    if (agris.length === 0) {
      sno = 0;
    }
